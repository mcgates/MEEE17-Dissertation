\begin{titlepage}
	\centering
		\vspace*{1cm}	
			
		{\Large \textbf{Modelling and Implementation of a DC microgrid Energy Management System}	}
		\vspace{0.5cm}	
		\\\relax
		By
		\\\relax	
		\vspace{0.5cm}
		\textbf{Fannie Isaac Masango\\}
		\vspace{2cm}		
		Submitted in partial fulfillment of the requirement for degree\\\relax
		\textbf{Master of Engineering in Electrical Engineering\\\relax}
		\vspace{0.8cm}
		in the\\	
		Department of Electrical Engineering\\\relax
		\vspace{1.5cm}
		FACULTY OF ENGINEERING AND THE BUILT ENVIRONMENT\\\relax
		\textbf{Tshwane University of Technology\\\relax}
		\vspace{1.5cm}
		Supervisor: Prof. J.A. Jordaan\\\relax
		Co-Supervisor: Dr A F Nnachi\par
		\vspace{2.5cm}
		October 2024
		\setcounter{page}{0}		%title page not numbered
\end{titlepage} 		%End of title page
